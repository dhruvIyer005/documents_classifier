"""
evaluate_deberta.py - Independent DeBERTa model evaluation

Standalone script to evaluate the fine-tuned DeBERTa model on all PDFs.
...
OUTPUT_CSV = OUTPUTS_DIR / "deberta_evaluation.csv"
...
logger.info(f"DeBERTa Model Path: {DEBERTA_MODEL_PATH}")

# ============ SETUP MODEL & TOKENIZER ============
try:
    logger.info("Loading DeBERTa model and tokenizer...")
    tokenizer = AutoTokenizer.from_pretrained(str(DEBERTA_MODEL_PATH))
    model = AutoModelForSequenceClassification.from_pretrained(str(DEBERTA_MODEL_PATH))
    model.to(DEVICE)
    model.eval()
...
def evaluate():
    ...
    logger.info("\n" + "=" * 60)
    logger.info("DEBERTA EVALUATION RESULTS")
    logger.info("=" * 60)
    ...
    logger.info("SUMMARY")
    logger.info("=" * 60)
    logger.info("Model: DeBERTa (microsoft/deberta-base)")
